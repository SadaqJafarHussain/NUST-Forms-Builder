        ...
        <div className="my-2 flex w-full px-2 text-sm text-slate-400">
          {status === "welcomeCard" && (
            <div className="mb-2 flex">
              ...
              <div className="mr-6 flex flex-col text-slate-700">بطاقة الترحيب</div>
            </div>
          )}
          {status === "skipped" && (
                ...
                {skippedQuestions.length > 1 && (
                  <TooltipProvider delayDuration={50}>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p data-testid="tooltip-respondent_skipped_questions">تم تخطّي هذه الأسئلة</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                )}
              </div>
              <div className="mr-6 flex flex-col text-slate-400">
                {skippedQuestions?.map((questionId) => {
                  return (
                    <p className="my-1 text-xs line-through" key={questionId}>
                      {parseRecallInfo(
                        getLocalizedValue(
            ...
            <div className="flex w-full">
              ...
              <div className="mb-2 mr-4 flex flex-col">
                <p
                  ...
                  className="mb-2 w-fit rounded-lg bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">
                  توقّف عند هذا السؤال
                </p>
                {skippedQuestions &&
                  skippedQuestions.map((questionId) => {
                    return (
                      <p className="my-1 text-xs text-slate-400" key={questionId}>
                        {parseRecallInfo(
                          getLocalizedValue(
                            questions.find((question) => question.id === questionId)!.headline,
                            "default"
